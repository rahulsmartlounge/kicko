...
function setDefaultAddress(id) {
    $.post("<?= base_url('profile/setDefaultAddress') ?>", { add_Id: id }, function (res) {
        if (res.status === 'success') {
            ...
            // Loop through all address cards and rebuild actions
            $('[id^="address_card_"]').each(function () {
                const $card = $(this);
                const cardId = $card.attr('id').replace('address_card_', '');
                const isDefault = parseInt(cardId) === parseInt(id);

                // Build actions dynamically
                let actions = `
                    <a href="javascript:void(0)" onclick="editAddress(${cardId})">Edit</a> |
                    <a href="#" onclick="openDeleteModal(${cardId})">Remove</a> |
                `;

                if (isDefault) {
                    actions += `<span>Default</span>`;
                } else {
                    actions += `<a href="javascript:void(0);" onclick="setDefaultAddress(${cardId})">Set as Default</a>`;
                }

                // Replace the entire action section
                $card.find('.mt-2').html(actions.trim());
            });
